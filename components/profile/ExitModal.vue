...
<template>
  <TransitionRoot as="template" :show="profile.openExitModal">
    <Dialog as="div" class="relative z-10" @close="profile.openExitModal = false">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        ...
        >
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            ...
            >
              <img :src="getIcon + 'close-btn.png'" @click="profile.openExitModal = !profile.openExitModal" class="close-img" />
                ...
                <div>
                  <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left flex flex-col gap-8">
                    <div class="flex flex-col gap-3">
                      <p class="modal-title">{{ $t('log_out_modal.exit') }}</p>
                      <p class="warning-text">{{ $t('log_out_modal.question') }}</p>
                    </div>
                    <div class="flex justify-center items-center gap-8">
                      <button class="no-exit">{{ $t('log_out_modal.no') }}</button>
                      <button class="exit-bth" @click="auth.signOut()">{{ $t('log_out_modal.yes') }}</button>
                    </div>
                  </div>
                </div>
              </div>
